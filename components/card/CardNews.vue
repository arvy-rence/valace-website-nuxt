    ...
    <div class="w-[22rem] bg-gray mx-4 ">
        <div class="block bg-white-100 custom-bottom-shadow img-with-shadow my-[.7rem] px-[1.4rem] pt-4 h-[32rem]">
            ...
            <figure class="flex justify-center items-center">
                <img :src="newsInfo.newsImageURL" alt="" class="w-[20rem] h-[12rem] object-cover"/>
            </figure>
            <div class="text-gray-400 font-kulim pt-4">
                <i class="fa-regular fa-calendar"></i> {{ dummyDateString }}
            </div>
            <div class="block overflow-hidden h-[10rem]">
                <h1 class="font-bold text-xl text-black-100 py-2 text-left font-kulim overflow-hidden max-h-[4.3rem]">{{ newsInfo.newsTitle }}</h1>
...
export default {
    ...
    props: {
        /**
         * newsInfo should have the following format:
         *     newsId: number (not required in this component)
         *     newsTitle: string;
         *     newsDate: Date;
         *     newsContent: string;
         *     newsImageURL: string;
    ...
    computed: {
        dateString() {
            return this.newsInfo['newsDate'].toDateString()
        },
        dummyDateString() {
            return this.dummyDate.toDateString()
        }
    }
}
</script>
